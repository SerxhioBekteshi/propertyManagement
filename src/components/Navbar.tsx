...
import { Building2, LogOut, Plus, ChevronDown, User, Shield } from 'lucide-react';
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="hidden sm:block">
            <span className="text-base font-semibold text-slate-900 tracking-tight">EstateFlow</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white text-sm font-medium px-4 py-2 rounded-xl transition-all duration-200"
          >
                  ...
                  {profile?.country && (
                    <>
                      ...
                      <span className="text-xs text-slate-400">
                        {countryFlag[profile.country]} {profile.country.charAt(0).toUpperCase() + profile.country.slice(1)}
                      </span>
                    </>
                  )}
                </div>
